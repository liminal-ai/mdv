...
conversational guidance about what to do next. All of this also works on regular
folders, not just packages.
...
- Package operations through chat: create package, export package, modify manifest — the Steward principle applied to all Epic 9 UI operations
...
| A2 | Epic 9 (package viewer integration) is complete: package-mode sidebar, manifest parsing, package creation, export, mode switching | Unvalidated | Dev team | Epic 13 makes these operations chat-accessible |
...
| A5 | The CLI process's working directory is the effective workspace root, so the CLI's built-in Read tool also works on workspace files as a fallback mechanism alongside the curated script methods | Unvalidated | Tech Lead | Tech design confirms working directory configuration |
...
**AC-2.3:** File reading works in both package mode and folder mode

- **TC-2.3a: Read file in folder mode**
...
- **TC-2.3b: Read file in extracted package**
...
The Steward can perform package operations that previously required the UI.
This is the Steward principle: anything the developer can do in the viewer, they
can do through the chat. Package creation, manifest modification, package
export, and sidebar navigation are all available through the script execution
lane.
...
**AC-5.3:** The Steward's context includes spec-awareness instructions when a spec package is detected

- **TC-5.3a: Spec package recognized**
  - Given: A package manifest has `type: spec`
  - When: The server constructs the provider context
  - Then: The context includes spec-specific instructions informing the Steward about Liminal Spec methodology, available phases, and its role as a spec guide
- **TC-5.3b: Non-spec package has no spec instructions**
  - Given: A package manifest does not have `type: spec`
  - When: The server constructs the provider context
  - Then: No spec-specific instructions are included; the Steward operates as a general package-aware assistant
...
- **TC-6.1c: Full pipeline — implementation phase detected**
  - Given: A spec package contains PRD, epic, tech design, and story files
  - When: The developer sends a message
  - Then: The detected phase is `implementation`; the artifact list reflects all detected types
...
interface ProviderContext {
  ...
  package?: {                        // Present when workspace.type === 'package'
    sourcePath: string;             // Original .mpk/.mpkz path
    format: 'mpk' | 'mpkz';
...
the root" pattern). The `package` field is present only when a package is open.
The `spec` field is present only when the package has `type: spec` in its
...
|--------|----------------|-------------|
| `getFileContent(path)` | folder, package | Read any file in the workspace by relative path. Returns file content as string. Rejects paths that resolve outside the root. |
...
| `implementation` | Not detected from files — inferred when stories exist and `specPhase` is `implementation` |
...
1. **Multi-file context budget:** When the Steward reads multiple files via `getFileContent` during a single response, the accumulated content may exceed the CLI's context window. Should the server enforce a per-response file-read budget, or let the CLI's own context limits handle truncation?
...
6. **File creation in extracted packages:** When `addFile` creates a file in an extracted package temp directory, should the stale indicator (from Epic 9) update? How does the `chat:file-created` notification interact with the stale indicator?
...
10. **CLI working directory alignment:** Should the CLI process's working directory be set to the effective workspace root so its built-in Read/Write tools work alongside the curated script methods? Or should the script methods be the sole file access mechanism?
...
- AC-2.3 (file reading in both modes)
...
- AC-5.3 (spec-awareness instructions)
...
- [ ] Story breakdown covers all ACs (27 ACs mapped across Stories 0–6)
...
- [ ] Steward principle verified — all Epic 9 UI operations have chat equivalents
- [ ] Scope boundary with Epic 14 clear — no autonomous execution, no background tasks, no approval flow
- [ ] Script context methods follow tech arch coarse-grained product action recommendation
